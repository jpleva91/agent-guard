...
describe('formatHookResponse — enforcement modes', () => {
  ...
  const deniedResult = {
    ...
    events: [],
    runId: 'test-run',
  } as unknown as import('@red-codes/kernel').KernelResult;
  ...
  it('backward compatible: calling with just result still works (denied)', () => {
    const response = formatHookResponse(deniedResult);
    const parsed = JSON.parse(response);
    expect(parsed.hookSpecificOutput.permissionDecision).toBe('deny');
    expect(parsed.hookSpecificOutput.permissionDecisionReason).toContain('Force push not allowed');
  });

  it('backward compatible: calling with just result still works (allowed)', () => {
    const response = formatHookResponse(allowedResult);
    expect(response).toBe('');
  });

  describe('educate mode', () => {
    const educateOptions: HookResponseOptions = { mode: 'educate' };

    it('returns allow with additionalContext containing the suggestion', () => {
      const response = formatHookResponse(deniedResult, suggestion, educateOptions);
      const parsed = JSON.parse(response);
      expect(parsed.hookSpecificOutput.permissionDecision).toBe('allow');
      expect(parsed.hookSpecificOutput.additionalContext).toContain('[AgentGuard educate]');
      expect(parsed.hookSpecificOutput.additionalContext).toContain(suggestion.message);
      expect(parsed.hookSpecificOutput.additionalContext).toContain(
        'Suggested command: git push origin feature-branch'
      );
    });

    it('includes suggestion message without correctedCommand when absent', () => {
      const simpleSuggestion: Suggestion = { message: 'Avoid force push.' };
      const response = formatHookResponse(deniedResult, simpleSuggestion, educateOptions);
      const parsed = JSON.parse(response);
      expect(parsed.hookSpecificOutput.permissionDecision).toBe('allow');
      expect(parsed.hookSpecificOutput.additionalContext).toContain('Avoid force push.');
  ...
  describe('guide mode', () => {
    it('includes suggestion and correctedCommand in permissionDecisionReason', () => {
      const guideOptions: HookResponseOptions = {
        mode: 'guide',
        retryAttempt: 1,
        maxRetries: 3,
      };
      const response = formatHookResponse(deniedResult, suggestion, guideOptions);
      const parsed = JSON.parse(response);
      expect(parsed.hookSpecificOutput.permissionDecision).toBe('deny');
    ...
    it('includes attempt counter without suggestion', () => {
      ...
      const response = formatHookResponse(deniedResult, null, guideOptions);
      const parsed = JSON.parse(response);
      expect(parsed.hookSpecificOutput.permissionDecision).toBe('deny');
      const reason = parsed.hookSpecificOutput.permissionDecisionReason;
    ...
    it('guide mode on allowed result returns empty string', () => {
      ...
      const response = formatHookResponse(allowedResult, suggestion, guideOptions);
      expect(response).toBe('');
    });
  });
  ...
  describe('enforce mode', () => {
    it('omits suggestion fields and uses standard deny format', () => {
      ...
      const response = formatHookResponse(deniedResult, suggestion, enforceOptions);
      const parsed = JSON.parse(response);
      expect(parsed.hookSpecificOutput.permissionDecision).toBe('deny');
      const reason = parsed.hookSpecificOutput.permissionDecisionReason;
      expect(reason).toContain('Force push not allowed');
  ...
  describe('monitor mode', () => {
    it('returns empty string for allowed actions', () => {
      ...
      const response = formatHookResponse(allowedResult, suggestion, monitorOptions);
      expect(response).toBe('');
    });

    it('uses standard deny format for denied actions', () => {
      const monitorOptions: HookResponseOptions = { mode: 'monitor' };
      const response = formatHookResponse(deniedResult, suggestion, monitorOptions);
      const parsed = JSON.parse(response);
      expect(parsed.hookSpecificOutput.permissionDecision).toBe('deny');
