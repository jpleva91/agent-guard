...
// Follows the EVENT_SCHEMAS pattern from events.js.
// No DOM, no Node.js APIs — pure data definitions.

export const MODULE_CONTRACTS = {
  'domain/battle': {
    exports: {
      calcDamage: { params: ['attacker', 'move', 'defender', 'typeChart', 'rng'], returns: 'DamageResult' },
      calcHealing: { params: ['move', 'bugmon'], returns: 'object' },
      resolveMove: { params: ['attacker', 'move', 'defender', 'typeChart', 'rng'], returns: 'MoveResult' },
      createBattleState: { params: ['playerMon', 'enemyMon'], returns: 'BattleState' },
      getTurnOrder: { params: ['playerMon', 'enemyMon'], returns: 'string' },
      executeTurn: { params: ['state', 'playerMove', 'enemyMove', 'typeChart', 'rolls'], returns: 'object' },
      simulateBattle: { params: ['monA', 'monB', 'movesData', 'typeChart', 'maxTurns', 'options'], returns: 'object' },
  ...
  'domain/evolution': {
    exports: {
      ...
      checkEvolution: { params: ['monster', 'events', 'evolutionData', 'monstersData'], returns: 'object|null' },
      checkPartyEvolutions: { params: ['party', 'events', 'evolutionData', 'monstersData'], returns: 'object|null' },
      applyEvolution: { params: ['oldMon', 'evolvedForm'], returns: 'object' },
      getEvolutionProgress: { params: ['monster', 'events', 'evolutionData', 'monstersData'], returns: 'object|null' },
    },
    invariants: [
 ...
 * @param {string} moduleName - Key in MODULE_CONTRACTS
 * @param {object} moduleExports - The module's actual exports (e.g., from import)
 * @returns {{ valid: boolean, errors: string[] }}
 */
export function validateContract(moduleName, moduleExports) {
  const errors = [];
    ...
    if (!(exportName in moduleExports)) {
      ...
      if (spec.returns === 'class') {
        if (typeof exported !== 'function') {
          errors.push(`${moduleName}.${exportName} expected class/constructor, got ${typeof exported}`);
        }
