...
function formatSuggestionsAsYaml(
  suggestions: import('@red-codes/storage').PolicySuggestion[]
): string {
  const lines: string[] = [
    '# AgentGuard policy suggestions (auto-generated by `agentguard policy suggest`)',
    'rules:',
  ];
  for (const s of suggestions) {
    if (s.type === 'allow_rule' || s.type === 'scope_expansion') {
      lines.push(`  - action: ${s.actionType}`);
      lines.push(`    decision: allow`);
      lines.push(`    # ${s.type} — confidence ${(s.confidence * 100).toFixed(0)}%`);
      lines.push(`    # ${s.description.replace(/\n/g, ' ')}`);
    } else if (s.type === 'threshold_adjustment') {
      lines.push(
        `  # threshold_adjustment for ${s.actionType} — confidence ${(s.confidence * 100).toFixed(0)}%`
      );
      lines.push(`  # ${s.description.replace(/\n/g, ' ')}`);
    }
  }
...
function formatSuggestionsAsMarkdown(
  suggestions: import('@red-codes/storage').PolicySuggestion[],
  patterns: import('@red-codes/storage').DenialPattern[],
  sessionCount: number
): string {
  const lines: string[] = [
    '# AgentGuard Policy Suggestions',
    '',
    `Analyzed ${patterns.length} denial pattern(s) across ${sessionCount} session(s).`,
    '',
  ];

  if (suggestions.length === 0) {
    lines.push('No actionable suggestions at current confidence thresholds.');
    return lines.join('\n') + '\n';
  }

  lines.push('## Suggested Policy Changes', '');
  for (const s of suggestions) {
    const confidence = (s.confidence * 100).toFixed(0);
    lines.push(`### \`${s.actionType}\` — ${s.type} (${confidence}% confidence)`, '');
    lines.push(s.description, '');
    if (s.type === 'allow_rule' || s.type === 'scope_expansion') {
      lines.push('```yaml', `  - action: ${s.actionType}`, '    decision: allow', '```', '');
    }
  }
  return lines.join('\n') + '\n';
}

function renderSuggestTerminal(
  suggestions: import('@red-codes/storage').PolicySuggestion[],
  patterns: import('@red-codes/storage').DenialPattern[],
  sessionCount: number
): void {
  process.stderr.write('\n');
  process.stderr.write(`  ${bold('Policy Suggestions')}\n`);
  ...
  if (suggestions.length === 0) {
    process.stderr.write(
      `  ${dim('No actionable suggestions at current confidence thresholds.')}\n\n`
    );
    return;
  }
  ...
  for (const s of suggestions) {
    ...
    const typeLabel =
      s.type === 'allow_rule'
        ? color('allow_rule', 'green')
        : s.type === 'scope_expansion'
          ? color('scope_expansion', 'yellow')
          : s.type === 'threshold_adjustment'
            ? color('threshold_adjustment', 'cyan')
            : dim(s.type);
    process.stderr.write(`  ${bold(s.actionType)} — ${typeLabel} (${confidence}% confidence)\n`);
    process.stderr.write(`    ${dim(s.description)}\n`);
    if (s.type === 'allow_rule' || s.type === 'scope_expansion') {
      process.stderr.write(`    ${dim('Add to policy:')}\n`);
      process.stderr.write(`      ${dim(`- action: ${s.actionType}`)}\n`);
      process.stderr.write(`        ${dim('decision: allow')}\n`);
    }
    process.stderr.write('\n');
  }
}
...
async function policySuggest(args: string[]): Promise<number> {
  const parsed = parseArgs(args, {
    ...
    string: ['--format', '-f', '--dir', '-d', '--min-cluster'],
    alias: { '-f': '--format', '-d': '--dir' },
  });

  const jsonOutput = parsed.flags['json'] === true;
  const yamlOutput = parsed.flags['yaml'] === true || parsed.flags['format'] === 'yaml';
  const markdownOutput =
    parsed.flags['markdown'] === true ||
    parsed.flags['md'] === true ||
    parsed.flags['format'] === 'markdown';
  const minCluster = parseInt((parsed.flags['min-cluster'] as string) ?? '2', 10) || 2;
  const baseDir = (parsed.flags['dir'] as string) ?? '.agentguard';
  ...
  try {
    ...
    // SQLite unavailable — inform user and exit gracefully
    process.stderr.write('\n  No governance database found.\n');
    process.stderr.write(
      `  Run \`agentguard guard --store sqlite\` to start recording sessions.\n`
    );
    process.stderr.write(`  Session data is stored in: ${baseDir}/\n\n`);
    process.stderr.write(`  ${dim('Advanced cloud suggestions: https://agentguard.dev')}\n\n`);
    return 0;
  }
  ...
  // Run analysis and filter by min-cluster
  const { patterns } = analyzeDenialPatterns(denialEvents, allCrossRunEvents);
  const filteredPatterns = patterns.filter((p) => p.occurrences >= minCluster);
  // Re-derive suggestions from filtered patterns only
  const { suggestPolicyChanges } = await import('@red-codes/storage');
  const filteredSuggestions = suggestPolicyChanges(filteredPatterns);
  ...
  if (yamlOutput) {
    process.stdout.write(formatSuggestionsAsYaml(filteredSuggestions));
    return 0;
  }

  if (markdownOutput) {
    process.stdout.write(
      formatSuggestionsAsMarkdown(filteredSuggestions, filteredPatterns, distinctSessions)
    );
    return 0;
  }

  renderSuggestTerminal(filteredSuggestions, filteredPatterns, distinctSessions);
  process.stderr.write(
    `  ${dim('Tip: Use --yaml to generate copy-pasteable policy rules, or --json for machine output.')}\n`
  );
  process.stderr.write(`  ${dim('Advanced cloud suggestions: https://agentguard.dev')}\n\n`);
  return 0;
}
