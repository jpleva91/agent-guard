...
import { computeAllRunRiskScores } from '@red-codes/analytics';
...
export async function analytics(args: string[], storageConfig?: StorageConfig): Promise<number> {
  ...
  if (storageConfig?.backend === 'sqlite') {
    ...
    const { clusterViolations } = await import('@red-codes/analytics');
    const { computeAllTrends } = await import('@red-codes/analytics');
    ...
    const sessionEventsMap = new Map<string, import('@red-codes/core').DomainEvent[]>();
...
function formatQueryTerminal(query: QueryName, result: unknown): void {
  switch (query) {
    case 'top-denied': {
      const rows = result as import('@red-codes/storage').TopDeniedAction[];
      if (rows.length === 0) {
    ...
    case 'violation-rate': {
      const rows = result as import('@red-codes/storage').ViolationTimeBucket[];
      if (rows.length === 0) {
        process.stderr.write('\n  No violations found.\n\n');
        return;
      }
    ...
    case 'session-stats': {
      const rows = result as import('@red-codes/storage').SessionSummary[];
      if (rows.length === 0) {
        process.stderr.write('\n  No sessions found.\n\n');
        return;
      }
