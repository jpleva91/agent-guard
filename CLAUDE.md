...
├── tests/                  # Test suite (77 JS + 15 TS test files)
...
2. **Contract** — Define module contracts in `src/domain/contracts.ts` (exports, invariants, dependencies)
3. **Shapes** — Define input/output shapes in `src/domain/shapes.ts` if new data structures are introduced
...
npm run ts:test                        # Run TypeScript tests (15 test files, vitest)
...
- The canonical event model (`src/domain/events.ts`) is the architectural spine — specs must reference it
