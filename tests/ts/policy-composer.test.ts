...
import { composePolicies, describeComposition } from '../../src/policy/composer.js';
import type { CompositionSource } from '../../src/policy/composer.js';
...
describe('PolicyComposed event', () => {
  it('creates a valid PolicyComposed event', async () => {
    const { createEvent, POLICY_COMPOSED, resetEventCounter } =
      await import('../../src/events/schema.js');
...
describe('composition with evaluator', () => {
  it('composed policies are evaluated correctly', async () => {
    const { evaluate } = await import('../../src/policy/evaluator.js');
