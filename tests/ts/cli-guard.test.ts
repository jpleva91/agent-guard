...
describe('guard command kernel integration', () => {
  it('can create kernel with dry-run and no policy', async () => {
    ...
    const { createKernel } = await import('../../src/kernel/kernel.js');
    const { resetActionCounter } = await import('../../src/core/actions.js');
    const { resetEventCounter } = await import('../../src/events/schema.js');
